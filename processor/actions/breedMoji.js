...
const combineDna = (sire, breeder, prng) => {
    ...
    return sireGenes
        ...
        .map(([sireGene, breederGene]) => {
            // console.log(sireGene, breederGene)
...
const breedMoji = (context, publicKey, sire, breeder, signature) => {
    ...
    if (!isValidAddress(sire)) {
        throw new InvalidTransaction('Sire address must be a 70-char hex string: ' + sire);
    }
    if (!breeder) {
        throw new InvalidTransaction("breeder doesn't exists");
    }
    if (!isValidAddress(breeder)) {
        throw new InvalidTransaction('Breeder address must be a 70-char hex string:' + breeder);
    }

    let owner = getCollectionAddress(publicKey);
    const prng = getPrng(signature);

    return context.getState([owner, sire, breeder])
        .then(state => {
            if (state[owner].length === 0) {
                throw new InvalidTransaction('Collection not found: ' + owner);
            }
            if (state[sire].length === 0) {
                throw new InvalidTransaction('Sire not found: ' + sire);
            }
            if (state[breeder].length === 0) {
                throw new InvalidTransaction('Breeder not found: ' + breeder);
            }
            ...
            if (breederState.owner !== publicKey) {
                throw new InvalidTransaction('Breeder must be owned by signer: ' + breeder);
            }
            ...
            return context.getState([listing])
                .then(state => {
                    ...
                    if (isNotListed) {
                        throw new InvalidTransaction('Sire is not listed: ' + sire);
                    }
